...
export class CurrencyL1 {
  ...
  async startInitialValidatorCl1() {
    ...
    const { url: allowanceListUrl, fileName: allowanceListFileName } =
      await this.allowanceListService.buildAllowanceListformation(Layers.CL1);
  ...
  async startValidatorNodeCl1() {
    ...
    const args = [
      `nohup java -jar currency-l1.jar run-validator`,
      `--ip ${this.currentNode.ip}`,
      url ? `--seedlist ${fileName}` : '',
      allowanceListUrl ? `--allowanceList ${allowanceListFileName}` : '',
      `> currency-l1-startup.log 2>&1 &`,
